    ...
    <div className='flex flex-col items-center gap-4 py-16 text-gray-800' id='specialty'>
        ...
        <div className='flex sm:justify-center gap-4 pt-5 overflow-scroll'>
            {specialityData.map((specialty, i) => (
                <Link onClick={()=>{scrollTo(0,0)}} className='flex flex-col items-center text-xs cursor-pointer flex-shrink-0 hover:translate-y-[-10px] transition-all duration-500' key={i} to={`/doctors/${specialty.speciality}`}>
